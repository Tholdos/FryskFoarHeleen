<template>
  <div class="flashcard-container">
    <div v-if="sentenceStore.loading" class="loading">
      Loading sentences...
    </div>

    <div v-else-if="currentSentence" class="flashcard-wrapper">
      <!-- Direction Toggle -->
      ...
      >
        ...
        <template v-if="dutchToFrisian">
          <div class="card-front">
            ...
            <div class="sentence">{{ currentSentence.dutch }}</div>
            <div class="hint">Klik om te draaien</div>
          </div>
          <div class="card-back">
            <div class="language-label">Fries</div>
            <div class="sentence">{{ currentSentence.frisian }}</div>
            <div class="pronunciation">Uitspraak: {{ currentSentence.pronunciation }}</div>
            <button 
              @click.stop="playAudio" 
              class="audio-icon"
              :disabled="isPlaying"
              :title="currentSentence.audioUrl ? 'Speel uitspraak af' : 'Audio nog niet beschikbaar'"
            >
    ...
    <div v-else class="no-sentences">
      Geen zinnen beschikbaar. Start de backend server.
    </div>
  </div>
</template>
...
function nextCard() {
  // If card is flipped, flip it back first before changing sentence
  if (isFlipped.value) {
    isFlipped.value = false
    // Wait for flip animation to complete (0.6s) before loading next sentence
    setTimeout(() => {
...
function playAudio() {
  ...
  if (currentSentence.value?.audioUrl) {
    const API_URL = import.meta.env.VITE_API_URL || 'http://localhost:3000'
    const fullAudioUrl = `${API_URL}/api/audio/${currentSentence.value.audioUrl}`
    ...
    console.log('Audio not available for this sentence yet')
  }
}
</script>
